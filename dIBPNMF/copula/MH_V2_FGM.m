function [v] = MH_V2_FGM(v, alpha, k, Yn, nidx, Z2n, V2n, A, epsilon)
...
%         tic;
        ...
        log_p_xstar     = log_likelihood( xStar, TMP, Yn, nidx, Ak, epsilon);

        log_p_st1       = log_likelihood( x_t_1, TMP, Yn, nidx, Ak, epsilon);
...
function f = log_likelihood(x_tmp, TMP, Yn, nidx, Ak, epsilon)

    TMP2 = TMP + x_tmp * Ak;

%     f  = sum(log(exppdf(Yn, A * transpose(Z2n .* V2n) + epsilon) + eps  ) );     
          
    f = sum( -log(TMP2(nidx) + epsilon) - Yn(nidx) ./ (TMP2(nidx) + epsilon) );
end
