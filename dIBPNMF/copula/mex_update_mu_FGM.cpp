...
double log_likelihood_mu(double* xStar, double* mu_k_minus, double* mu_k_plus, int Z1sum, int Z2sum,int N1, int N2, double A, double B, int k, int K, double RHO)
{
    ...
    if(k == (K-1)){
        ...
        fl = fl + log(1 + RHO*(2* pow(xstart_minus1, A) - 1)*(2* pow(xstart_minus2, B) - 1) + 0.0000000001)  ;
        ...
        fl = fl + log(1 + RHO*(2* pow(plus_xstart1, A) - 1)*(2* pow(plus_xstart2, B) - 1) + 0.0000000001)   ;
...
// MH_mu(mu_k_tmp, k, K, mu_k_minus, mu_k, mu_k_plus, (int) *(Z1sum + k), (int) *(Z2sum + k), N1, N2, A, B, RHO);

void MH_mu(double* returned,int k, int K, double* mu_k_minus, double* mu_k, double* mu_k_plus, int Z1sum, int Z2sum, int N1, int N2, double A, double B, double RHO)
{
//     if (k <= 10)
//     mexPrintf("in MH_mu k = %d ....... mu1(k-1) = %f, mu1(k+1) = %f, mu2(k-1) = %f, mu2(k+1) = %f. \n", k, *(mu_k_minus), *(mu_k_plus), *(mu_k_minus+1), *(mu_k_plus+1));
	...
	while(num < nSamples){
        ...
        // log_likelihood_mu(xStar, mu_k_minus, mu_k_plus, Z1sum, Z2sum, N1, N2, A, B, k, K, RHO)
		log_p_xstar = log_likelihood_mu(xStar, mu_k_minus, mu_k_plus, Z1sum, Z2sum, N1, N2, A, B, k, K, RHO);
    
		log_p_st1   = log_likelihood_mu(x_t_1, mu_k_minus, mu_k_plus, Z1sum, Z2sum, N1, N2, A, B, k, K, RHO);
    ...
    *(returned + 1) = samples[1][selected];
    ...
    if (selected >= nSamples || selected < 0)
        mexPrintf(" selected = %d. \n", selected);
    
    if ((samples[0][selected] < *(mu_k_plus) || samples[0][selected] > *(mu_k_minus)) && samples[0][selected] > 0.0000001)
        mexPrintf("prblem 1 ! k = %d ....... mu(k-1) = %f, new mu(k) = %f, mu(k+1) = %f. \n", k, *(mu_k_minus), samples[0][selected], *(mu_k_plus));
    
    if ((samples[1][selected] < *(mu_k_plus+1) || samples[1][selected] > *(mu_k_minus+1)) && samples[1][selected] > 0.0000001)
        mexPrintf("prblem 2 ! k = %d ....... mu(k-1) = %f, new mu(k) = %f, mu(k+1) = %f. \n", k, *(mu_k_minus+1), samples[1][selected], *(mu_k_plus+1));
...
// update_mu(mu_new_in, MU, Z1sum, Z2sum, A, B, RHO, K, N1, N2 )

void update_mu(double* mu_new_in, double* MU, double* Z1sum, double* Z2sum, double A, double B, double RHO, int K, int N1, int N2 )
{
...
// [mu] = Update_mu(mu, Z1sum, Z2sum, K_max, N1, N2, alpha(1), alpha(2), rho)
